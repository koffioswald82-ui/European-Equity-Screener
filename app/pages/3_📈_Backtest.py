...
with st.sidebar:
    ...
    benchmark   = st.text_input("Benchmark ticker", value="EXW1.DE",
                                help="EXW1.DE = iShares Euro Stoxx 50 ETF")
    period      = st.selectbox("History", ["2y", "3y", "5y"], index=1)
    rebal_freq  = st.selectbox("Rebalance", ["ME", "QE", "YE"], index=0,
...
with st.spinner("Running walk-forward backtest..."):
    bt = run_backtest(prices=prices, weights_fn=weights_fn,
                     benchmark_ticker=benchmark, rebalance_freq=rebal_freq,
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=bt.portfolio_nav.index, y=bt.portfolio_nav,
    name="Strategy", line=dict(color="#1B2A4A", width=2)))
fig.add_trace(go.Scatter(x=bt.benchmark_nav.index, y=bt.benchmark_nav,
...
fig.update_layout(yaxis_title="NAV (base 100)", height=450, hovermode="x unified",
...
fig2 = go.Figure()
fig2.add_trace(go.Scatter(x=dd_strat.index, y=dd_strat, fill="tozeroy",
    name="Strategy", line=dict(color="#1B2A4A"), fillcolor="rgba(27,42,74,0.2)"))
fig2.add_trace(go.Scatter(x=dd_bench.index, y=dd_bench, fill="tozeroy",
    name="Benchmark", line=dict(color="#B8962E", dash="dash"), fillcolor="rgba(184,150,46,0.15)"))
...
export_df = pd.DataFrame({"strategy_return": bt.portfolio_returns,
